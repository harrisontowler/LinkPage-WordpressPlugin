...
class Rewrites {
    ...
    add_rewrite_rule( $this->custom_store_url . '/([^/]+)/links?$', 'index.php?' . $this->custom_store_url . '=$matches[1]&links=true', 'top' );
add_rewrite_rule( $this->custom_store_url . '/([^/]+)/links/page/?([0-9]{1,})/?$', 'index.php?' . $this->custom_store_url . '=$matches[1]&paged=$matches[2]&links=true', 'top' );
    ...
    if ( get_query_var( 'links' ) ) {
        ...
        if ( ! empty( $store_name ) ) {
            ...
            return dokan_locate_template( 'store-toc1.php' );
        }
    }
